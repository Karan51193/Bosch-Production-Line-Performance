...
def date_new_features(df,file,is_train=None):
  ...
  if is_train:
    ...
    for chunk in pd.read_csv(file,usecols=['Id'] + date_cols,chunksize=50000,low_memory=False):
...
def date_final(df,file,is_train=None):
  import pandas as pd
  df_date_encoded_final=date_step1(df)#Pre-process and encode dates
  minmaxfeatures,data=date_new_features(df,file,is_train=is_train)#Additionally engineered features
...
def pre_process_date(file):
  ...
  for chunk in pd.read_csv(file,chunksize=rowcount//10):
    try:
    ...
    chunk_processed=date_final(chunk,file,is_train=False)
